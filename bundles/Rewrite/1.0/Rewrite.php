...
SB\Rewrite::add_rewrite_rule(array(
	'uri' => 'who-we-are/?',
	'template' => 'whoweare',
	'page'	=> array(
		'title'		=> 'Who we are',
...
class Rewrite
{
	...
	{
		self::do_rewrite_tags();
		self::do_rewrites();
		add_filter('template_include', array(__CLASS__, 'template_include'));
	}

	public static function add_rewrite_rule($args)
	{
		$defaults = array(
			'uri' => '',
			'path' => '',
			...
			'template' => '',
			'is_archive' => false,
			'after' => 'bottom' // This can either be 'top' or 'bottom'. 'top' will take precedence over WordPress's existing rules, where 'bottom' will check all other rules match first. Default: "bottom"
		);
	...
	private static function do_rewrites()
	{
	 ...
	 */
	private static function do_rewrite_tags()
	{
	...
	{
		if (empty(self::$page_data['page']['title'])) return $title;
	...
	public static function custom_template()
	{

		if (empty(self::$page_data['template'])) return false;
		return self::$page_data['template'];

	}

	public static function is_custom_template($template)
	{

		if (empty(self::$page_data['template'])) return false;

		if ($template == self::$page_data['template']) return true;
		return false;
	}
