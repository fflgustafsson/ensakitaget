...
class Sortable
{
    ...
    public static function form()
    {
        ...
        $category_select = ($by_category) ? self::getCategorySelect($post_type, $category_label, $taxonomy) : false;
        $language_select = ($language) ? self::getLanguageSelect($post_type, $language_label) : false;
        ...
        <div class="<?php echo implode(' ', $classes) ?>">
            ...
            <form method="post" id="page-order-form" class="options sb-page-order">
                ...
                <div class="page-wrapper">
                    <?php self::printList($by_category, $taxonomy, $language); ?>
                </div>
                <?php self::sortHelper(); ?>
                <p class="submit">
    ...
    public static function getPagesHierachy($page_id = 0, $by_category = false, $taxonomy = false, $category = false, $language = false)
    {
        ...
        if ($language) {
            $query['meta_query'] = array(array(
                'key' => \SB\Language::$meta_key,
                'value' => Utils::getString('lang')
                ));
        }
        ...
        foreach ($pages as $i => $page) {
            $return[$page->ID] = array(
                ...
                'children' => self::getPagesHierachy($page->ID, $by_category, $taxonomy, $category, $language),
    ...
    public static function printList($by_category, $taxonomy, $language)
    {

        $category = Utils::getString('term_id');
        $language = (!$language) ? false : Utils::getString('lang');

        $pages = self::getPagesHierachy(0, $by_category, $taxonomy, $category, $language);
    ...
    public static function getCategorySelect($post_type, $label, $taxonomy)
    {
        ...
        $return[] = '<option value="" '.selected($selected, '', false).'>'.$label.'</option>';
    ...
    public static function getLanguageSelect($post_type, $label)
    {

        $languages = \SB\Language::getLanguages();

        $selected = Utils::getString('lang');
        $base_url = admin_url('edit.php?post_type='.$post_type.'&page=sb_'.$post_type.'_sort');

        $return = array();
        $return[] = '<select class="language-select" data-base-url="'.$base_url.'">';
        $return[] = '<option value="" '.selected($selected, '', false).'>'.$label.'</option>';

        foreach ($languages as $code => $language) {
            $return[] = '<option value="'.$code.'" '.selected($selected, $code, false).'>';
