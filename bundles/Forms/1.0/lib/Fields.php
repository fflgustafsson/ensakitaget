...
class Fields {
	...
	// FIXME Debug value consoles possible args

	public static function text($args)
	{

		$args = Common::validate_args(__FUNCTION__, $args);
		extract($args);

		$value = \SB\Forms::auto_register($auto_value, $value, $name, false);

		$value = (empty($value)) ? $default : $value;
		$placeholder = (!empty($placeholder)) ? $placeholder : null;
		$disabled = Common::is_disabled($disabled);
		$required = Common::is_required($required);
		...
		$tag = sprintf('%s<input %s type="%s" name="%s" class="regular-text" value="%s" placeholder="%s" %s %s />', $count, $id, $type, $name, esc_attr($value), $placeholder, $disabled, $required);
	...
	public static function textarea($args)
	{
		...
		$tag = sprintf('%s<textarea %s name="%s" class="large-text" rows="%d" placeholder="%s" %s>%s</textarea>', $count, $id, $name, $rows, $placeholder, $disabled, $value);
	...
	public static function select($args)
	{
		...
		$tag = array();
		$tag[] = sprintf('<select %s name="%s" %s>', $id, $name, $disabled);
	...
	public static function date($args)
	{
		...
		if ($timestamp) {
			// use convert method
			$value = \SB\Forms::auto_register($auto_value, $value, $name, 'SB\Forms\Common::date_to_timestamp');
			// default
			$value = (!empty($default) && empty($value)) ? $default : $value;
			// convert
			$value = Common::timestamp_to_date($value, $format);
		} else {
			$value = \SB\Forms::auto_register($auto_value, $value, $name, false);
			$value = (!empty($default) && empty($value)) ? $default : $value;
		}

		$settings = Common::remove_base_args($args);
		\SB\Forms::$date_settings[$class] = $settings;

		$icon = '<span class="calendar-icon dashicons dashicons-calendar-alt"></span>';

		$tag = sprintf('<input %s type="text" name="%s" class="date-picker" value="%s" />%s', $id, $name, $value, $icon);
	...
	public static function image($args)
	{
		...
		$value = \SB\Forms::auto_register($auto_value, $value, $name, true);
		...
		// Check for deleted
		$value = Common::check_for_deleted_files($value);
		$image_elements = array();

		foreach ($value as $media_id) {
			$image_elements[] = Common::get_media_element($media_id, $image_size, $default, $remove, $name);
		}
	...
	public static function posts($args)
	{
		...
		if (!empty($value)) {
			...
			foreach ($value as $id) {
				$load_data_via_ajax = (Utils::is_ajax_request()) ? true : false; // this happens when widget is saved
				$selected_posts[] = Common::get_post_element($id, $remove, $name, $load_data_via_ajax, $meta_key, $image_meta);
			}
		}
		...
		$tag[] = '<div class="'.implode(' ', $wrapper_classes).'" '.Common::data_attr($data).'>';
		...
		if (!$list) {
			...
			// FILTER + LIST
			$tag[] = '<span class="spinner"></span><a href class="button button-secondary choose-post-from-list">'.$choose.'</a>';
	...
	public static function url($args)
	{
		...
		if (is_numeric($value['id'])) {
			$permalink = str_replace(Utils::wp_siteurl(), '', get_permalink($value['id']));
		...
		if (!is_numeric($value['id']) && !empty($value['id'])) {
			$custom_item = SB\NavMenu::get_custom_item_data($value['id']);
			if ($custom_item) {
				if ($custom_item['type'] == 'custom_post_type') {
					$permalink = str_replace(Utils::wp_siteurl(), '', get_post_type_archive_link($custom_item['id']));
				} else {
	...
	public static function custom($args)
	{
		...
		if (!$method) {
			debug($args);
			debug('ERROR function is not callable');
			return false;
		}
	...
	public static function password($args)
	{
		...
		$value = \SB\Forms::auto_register($auto_value, $value, $name, 'SB\Forms\Common::encryptPassword');
		$value = (empty($value)) ? $default : $value;

		if (!is_callable('mcrypt_decrypt')) {
			debug('ERROR You need to install Mcrypt to use password decryption.');
		} else {
			$value = Common::decrypt_password($value);
		}
	...
	public static function fader($args)
	{
		...
		$tag = array();
		$tag[] = '<div class="'.implode(' ', $classes).'" '.Common::element_data($config).'>';
