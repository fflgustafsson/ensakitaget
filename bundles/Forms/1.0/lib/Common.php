...
class Common {
	...
	public static function validate_args($field, $args)
	{
		...
		if (empty($args['name'])) return self::error(__FUNCTION__, $args);
		...
		$defaults = array(
			...
			'id'	=> null, // to-do
			'name'	=> $field,
			'label' => Utils::mb_ucfirst($args['name']),
	...
	public static function error($field, $args)
	{
		...
		debug('ERROR in field of type '.$field.' with following arguments');
		debug($args);
		return $args;

	}

	public static function is_disabled($string)
	{

		if ($string) return 'disabled';
		return false;
	...
	public static function remove_base_args($args)
	{
	...
	public static function get_media_element($media_id, $size = 'thumbnail', $default, $remove = '&ndash;', $name)
	{
		...
		if (empty($img_tag)) return '<li class="sb-media-attachment hidden" data-id="-1"><input type="hidden" name="'.$name.'[]" value="-1"></li>';
	...
	public static function check_for_deleted_files($value)
	{

		if (empty($value) || 0 > $value) return array();

		$return = array();
		foreach ($value as $id) {
			if (0 > $id) return $value;
			$p = get_post($id);
			if ($p) {
				$return[] = $id;
			}
	...
	public static function get_default($name)
	{
		...
		$default = false;
		foreach (\SB\Forms\Options::$instances as $data_set) {
	...
	public static function get_post_element($post_id, $remove = '&ndash;', $name = false, $load_data_via_ajax = false, $meta_key, $image_meta)
	{
		...
		$element[] = '<span class="post-meta">'.Utils::shorten($meta_value, 50).'</span>';
	...
	public static function get_url($name, $post_id = false, $default = true)
	{
		...
		if ($post_id === true) {
			debug('Possible misuse since function change', true);
		}

		if (is_numeric($post_id)) {
			$data = get_post_meta($post_id, $name, true);
		} else {
			$default = ($default) ? self::get_default($name) : false;
		...
		if (!empty($data['id']) && is_numeric($data['id'])) {
			return array('url' => str_replace(Utils::wp_siteurl(), '', get_permalink($data['id'])), 'target' => $target, 'text' => $data['text']);
		}
	...
	public static function get_option($name, $default = true) // Quickfix: if user wants to be able to return an empty field, set default to false
	{
	...
	public static function encrypt_password($password)
	{
		...
		if (!is_callable('mcrypt_encrypt')) {
			debug('ERROR You need to install Mcrypt to use password encryption.');
			return $password;
		}
	...
	public static function timestamp_to_date($date, $format)
	{
		...
		if (!empty($formats[$format])) {
			...
			debug('ERROR:: cant match date format: '.$format);
		}
	...
	public static function element_data($data_array)
	{
	...
	public static function data_attr($data)
	{
