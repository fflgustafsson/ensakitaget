;(function($){
	...
	function openMediaChooser(settings) {
        ...
        file_frame.on( 'select', function() {
			...
			$.each(attachments, function(i, val) {
				if ((i + 1) > fetchMaxNumber && maxNumber) return false;
	...
	// Color Picker
	if (typeof colorFields !== "undefined") {
		...
		$('#wpbody-content').on('click', '.color-box', function(e) {
			$(this).next().iris('toggle');
		});

		$('#wpbody-content').on('focus', '.color-picker', function(e) {
			$(this).iris('toggle');
		});

		$('#wpbody-content').on('click', function(e) {

			if ($(e.target).hasClass('color-box')) return;
			if ($(e.target).hasClass('color-picker')) return;
			if ($(e.target).hasClass('iris-palette')) return;
	...
	if ($('.sb-forms-sortable').length > 0) {
		...
		formsSortable.each(function(){
			...
			$('body').on('mouseenter', sortWrapper, function(e){
	...
	$('.sb-multi-wrapper').on('after-selected-remove', '.sb-selected-wrapper', function(e) {
		...
		if (0 < selectedDisplay.children().length) return;
	...
	var postSelect = {
		...
		searchPosts: function(){
			...
			$(document).on('keyup', 'input.sb-post-search', function(){
				...
				typeTimer = setTimeout(function(){
					...
					if (wrapper.data('keep-string') === currentValue) return false;
		...
		showSearch: function(data, results, loader, wrapper){
			...
			var self = this,
				shortenTitle = wrapper.data('shorten-title');
		...
		bindSearchList: function(){
			...
			$(document).on('click', '.search-results a.select-post, .post-list a.select-post', function(e){
				...
				var wrapper = $(this).parents('.sb-post-select-wrapper'),
					result = $(this).parents('.search-results').fadeOut('fast'),
				...
				if (ajax_fn === 'set_post_id') {
					...
					displayWrapper = $('.display-wrapper', wrapper);
		...
		bindSearchEvent: function() {
			...
			$(document).on('input', 'input.sb-post-search', function(e){
	...
	var indexMultiForm = function(){
		...
		$('.sb-field-wrapper').each(function(index, el) {

			var wrapper = $(this),
				name = wrapper.data('name');

			$('input[name^=' + name + ']', wrapper).each(function(i, input) {
		...
		setCurrentValue: function(){
			...
			faders.each(function(i, elem){
				...
				var faderWrapper = $(this).find('.wrapper'),
					...
					finalPos = Math.min(Math.max(parseInt(calculatedPos), 0), fader.wrapper.outerWidth() - fader.knobWidth);
			...
			faders.on('mousedown touchstart', function(e){
				...
				var faderWrapper = $(this).find('.wrapper'),
					...
					knob.parents('.wrapper').on('mousemove touchmove', function(e){
						...
						toPos = e.originalEvent.pageX + currentPos - fader.knobWidth;
						moveTo = Math.min(Math.max(parseInt(toPos), fader.minPos), fader.maxPos);
