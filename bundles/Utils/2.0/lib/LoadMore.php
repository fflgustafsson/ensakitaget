...
class LoadMore
{
    ...
    public static function request()
    {
        ...
        $config_name = Utils::postString('config');
        $config = self::getConfig($config_name);
        $paged = Utils::postString('paged');
        $meta_value = Utils::postString('meta_value');
        $category = Utils::postString('category');
    ...
    public static function query($config, $meta_value = false, $category_name = false)
    {
        ...
        $args = (is_array($config)) ? $config : self::getConfig($config);
        ...
        if ($category) {
            unset($args['category']); // = false;
            unset($args['category_name']); // = false;
            unset($args['taxonomy']);
