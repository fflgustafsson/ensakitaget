...
class NavMenu
{
    ...
    public static function addNavItem($args)
    {
        $defaults = array(
            'type' => 'uri',
    ...
    public static function init()
    {
        ...
        add_filter('wp_setup_nav_menu_item', array(__CLASS__, 'setupNavItem'));
        add_filter('wp_nav_menu_objects', array(__CLASS__, 'makeCurrent'));
        add_action('wp_ajax_navmenu_add_custom', array(__CLASS__, 'ajaxAddCustom'));
    }

    public static function javascript()
    {
        wp_register_script('sb-navmenu', Utils::getBundleUri('NavMenu').'/js/navmenu.js', 'jquery', '1', true);
    ...
    public static function ajaxAddCustom()
    {

        $nonce = $_REQUEST['nonce'];

        if (!wp_verify_nonce($nonce, 'sb_navmenu')) {
            error_log('Can not verify ajaxAddCustom');
            die();
        }
        ...
        foreach ($nav_items as $nav_item) {
            if (!isset(self::$nav_items[$nav_item])) {
                continue;
            }
    ...
    public static function setupNavItem($menu_item)
    {
    ...
    public static function makeCurrent($items)
    {

        foreach ($items as $item) {
            if ($item->type !== 'sb_nav_item') {
                continue;
            }

            if (!isset(self::$nav_items[$item->object])) {
                continue;
            }
            ...
            if ('custom_post_type' == $nav_item['type']) {
                if (is_post_type_archive($nav_item['id']) || is_singular($nav_item['id'])) {
                    $item->current = true;
            ...
            if ($has_current) {
                $active_anc_item_ids = self::getItemAncestors($item);
    ...
    public static function getItemAncestors($item)
    {
        $anc_id = absint($item->db_id);

        $active_anc_item_ids = array();

        while ($anc_id = get_post_meta($anc_id, '_menu_item_menu_item_parent', true) &&
            !in_array($anc_id, $active_anc_item_ids)) {
    ...
    public static function getCustomItemData($item_id)
    {

        if (empty(self::$nav_items)) {
            return false;
        }
