...
class Analytics {
	 ...
	 * @return none
	 */
	public static function render_tag_manager() {
?>
		<!-- Google Tag Manager -->
		<noscript><iframe src="//www.googletagmanager.com/ns.html?id=<?php echo self::$options['tm_container_id']; ?>"
...
<?php
	 ...
	 * @return none
	 */
	public static function tag_manager_data() {
	...
	public static function google_analytics() {
		...
		add_action('wp_footer', array(__CLASS__, 'render_google_analytics'), 30);
	 ...
	 * @return none
	 */
	public static function render_google_analytics() {

		/*
		<script type="text/javascript">

		  var _gaq = _gaq || [];
		  _gaq.push(['_setAccount', <?php echo self::$options['ga_account']; ?>]);
...
<?php
	...
	public static function include_social_eventlisteners() {
		...
		wp_localize_script( 'sb-social-interactions', 'sbSocial', array('isGaInUse' => self::$is_google_analytics_used ? 'true' : 'false', 'isTagManagerInUse' => self::$is_tag_manager_used ? 'true' : 'false') );
	 ...
	 * post_types = optional default null string or array of post types used in conjunction with output = object to filter output
	 * days = int optional default 90 timespan of data in days from today
	 * count = int optional default 3 number of links to get. Note if filtering is set use a high count to sure you fetch all of a certain type
	 ...
	 */
	public static function get_top_visited( $options = array() ) {

		$params = array( 'output' => 'links', 'post_types' => null, 'days' => 90, 'count' => 3, 'force_google' => false, 'filters' => null);
		...
		if( $links === false || $force_google ) {
			...
			$result = self::make_google_request($days, $count * 50, 'ga:pageviews', 'ga:pagePath', null, $filters);
		...
		if( $output == 'object' ){
			...
			return self::get_top_unique_wpobj($links, $post_types, $count);
	 ...
	 * @return array permalinks|WPObjects
	 */
	public static function get_top_social_interactions( $options = array() ) {
		...
		if( $links === false || $force_google ) {
			...
			$result = self::make_google_request($days, $count * 50, 'ga:socialInteractions', array('ga:socialInteractionTarget', 'ga:socialInteractionAction'), null, $filters );
	 ...
	 */
	protected static function make_google_request( $days, $count, $metrics, $dimensions, $sort = null, $filters = null){
	 ...
	 */
	protected static function get_top_unique_wpobj($links, $post_types, $count)
	{
		...
		foreach ($links as $path => $hits) {
			...
			foreach ($post_types as $post_type) {
				$object = self::convert_path_to_wpobj($path, $post_type);
			}

			if ($object) {

				if ($object->post_status != 'publish') continue;
	 ...
	 * @return WP object || false
	 */
	protected static function convert_path_to_wpobj($path, $post_type)
	{
